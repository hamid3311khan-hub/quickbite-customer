...
const upload = multer();
const bcrypt = require('bcryptjs'); // <-- SIRF YE 1 LINE ADD KI
...
// CHANGE 3: REGISTER ROUTE UPDATE - File upload + Trial + HASH
...
app.post('/api/restaurant/register', uploadFile.single('payment_proof'), async (req,res)=>{
    try{
        ...
        const hashedPassword = await bcrypt.hash(password, 10); // <-- SIRF YE LINE ADD
        ...
        await new RestaurantOwner({
            ...
            password: hashedPassword, // <-- HASHED PASS
...
app.post('/api/restaurant/login', async (req,res)=>{
    try{
        ...
        const isMatch = await bcrypt.compare(password, owner.password); // <-- SIRF YE 2 LINE ADD
...
// CHANGE 4: APPROVE ROUTE UPDATE - Trial hi rakha
app.put('/api/restaurant/owner/:id/approve', async (req,res)=>{
    const owner = await RestaurantOwner.findByIdAndUpdate(req.params.id, {status: "Approved", plan_status: "Trial"}, {new:true}); // <-- Active ki jagah Trial
